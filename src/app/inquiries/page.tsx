...
import { inquiriesApi, projectsApi } from '@/lib/api';
import type { ClientInquiry, InquiryStatus, Project } from '@/lib/types';
import {
  Plus, Edit2, Trash2, ChevronDown, ChevronRight,
  MessageCircleQuestion, CheckCircle2, Clock, XCircle, FolderOpen, Sparkles, Mail,
} from 'lucide-react';
import clsx from 'clsx';
import { format } from 'date-fns';
import { ko } from 'date-fns/locale';
import { confirm } from '@/lib/confirm';
import InquiryAIModal from '@/components/inquiries/InquiryAIModal';
import InquiryAIExtractModal from '@/components/inquiries/InquiryAIExtractModal';
import InquiryRawEmailModal from '@/components/inquiries/InquiryRawEmailModal';
...
export default function InquiriesPage() {
  ...
  const [aiExtractProject, setAiExtractProject] = useState<number | null>(null);
  const [filterStatus,     setFilterStatus]     = useState('');
  const [filterProject,    setFilterProject]    = useState('');

  useEffect(() => {
    Promise.all([inquiriesApi.list(), projectsApi.list()])
      .then(([inqs, projs]) => { setItems(inqs); setProjects(projs); })
  ...
  async function handleSave(data: any) {
    if (editing) {
      ...
      const created = await inquiriesApi.create(data);
  ...
  function openAIExtract() {
    const pid = filterProject ? Number(filterProject) : (projects[0]?.id ?? null);
    setAiExtractProject(pid);
    setShowAIExtract(true);
  }

  const displayed = items
    .filter((i) => !filterStatus  || i.status === filterStatus)
    .filter((i) => !filterProject || String(i.project_id) === filterProject);
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-surface">
        ...
        {/* ── Header ── */}
        <header className="flex-shrink-0 border-b border-white/6 px-6 py-4 bg-surface-raised">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-sm font-semibold text-gray-100">고객문의</h1>
              <p className="text-xs text-gray-500 mt-0.5">고객사 질문 · 답변 통합 관리</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="h-9 inline-flex items-center gap-2 px-4 rounded-lg text-sm font-semibold bg-brand/15 text-brand hover:bg-brand/25 border border-brand/30 hover:border-brand/50 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              >
              ...
              <button
                onClick={openCreate}
                className="h-9 inline-flex items-center gap-2 px-4 rounded-lg text-sm font-semibold bg-white/8 text-gray-200 hover:bg-white/15 border border-white/12 hover:border-white/22 transition-all"
              >
          ...
          <div className="flex items-center gap-2 mt-3">
            {pending > 0 && (
              <span className="inline-flex items-center gap-1 text-xs font-medium text-amber-300 bg-amber-500/10 border border-amber-500/25 px-2.5 py-1 rounded-full">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            <div className="text-center py-20 text-gray-500 text-sm">불러오는 중...</div>
          ) : displayed.length === 0 ? (
            <div className="flex flex-col items-center gap-3 py-24 text-gray-600">
              <MessageCircleQuestion className="w-12 h-12 opacity-15" />
              <p className="text-sm">등록된 문의가 없습니다</p>
              <button
                onClick={openAIExtract}
                disabled={projects.length === 0}
                className="mt-1 inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold bg-brand/15 text-brand hover:bg-brand/25 border border-brand/30 transition-all disabled:opacity-40"
              >
                <Sparkles className="w-4 h-4" /> AI로 첫 문의 등록하기
              </button>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      {/* 프로젝트 뱃지 */}
                      {item.project_name && (
                        <span className="inline-flex items-center gap-1 text-xs text-blue-300 bg-blue-500/10 border border-blue-500/20 px-2 py-0.5 rounded-full shrink-0">
                      ...
                      <span className="text-xs text-gray-500 shrink-0 hidden sm:flex items-center gap-1.5">
                        {item.asked_by && <span className="text-gray-400">{item.asked_by}</span>}
                        {item.asked_by && <span className="text-gray-700">·</span>}
      ...
      {showModal && (
        <InquiryModal initial={editing} projects={projects} onClose={() => setShowModal(false)} onSave={handleSave} />
      )}
      {aiTarget && (
        <InquiryAIModal inquiry={aiTarget} onClose={() => setAiTarget(null)} onSave={handleAISave} />
      )}
      {showAIExtract && aiExtractProject && (
        <InquiryAIExtractModal
          projectId={aiExtractProject}
          projects={projects}
          onCreated={(item) => setItems((prev) => [item, ...prev])}
          onClose={() => { setShowAIExtract(false); setAiExtractProject(null); }}
        />
      )}
      {rawTarget && (
...
function InquiryModal({ initial, projects, onClose, onSave }: ModalProps) {
  const [saving,    setSaving]    = useState(false);
  const [projectId, setProjectId] = useState(initial ? String(initial.project_id) : '');
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSaving(true);
    try {
      await onSave({ project_id: Number(projectId), title, question, answer: answer || null, asked_by: askedBy || null, asked_at: askedAt, status: statusVal });
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-[#1a1d23] border border-white/10 rounded-2xl w-full max-w-xl shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/8">
          ...
          <button onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-lg text-gray-500 hover:text-gray-300 hover:bg-white/8 transition-all">✕</button>
        </div>

        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1.5">프로젝트 *</label>
            <select required value={projectId} onChange={(e) => setProjectId(e.target.value)} className="input w-full">
